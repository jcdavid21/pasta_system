              ...
              <li class="breadcrumb-item active">Sales</li>
                  ...
                  <div class="card mb-5">
                    <div class="card-header bg-secondary pt-3">
                        <div class="text-center">
                            <p class="card-title text-light">Pick up Pending Orders
                        </div>
                    </div>
                    <div class="card-body">
                      <table id="residenceAccounts" class="table table-striped nowrap" style="width:100%">
                        ...
                        <tbody>
                          <?php
                            ...
                            ta.contact, ta.address, ao.*, ot.*
                            FROM tbl_cart tc 
                            INNER JOIN tbl_products tp ON tp.prod_id = tc.prod_id 
                            INNER JOIN tbl_account_details ta ON ta.account_id = tc.account_id 
                            LEFT JOIN tbl_order_type ot ON ot.order_type_id = tc.order_type_id
                            LEFT JOIN tbl_add_ons ao ON ao.add_ons_id = tc.add_ons_id
                            WHERE tc.status_id = 3 AND ot.order_type_id = 1;";
                          ...
                          <tr>
                            ...
                            <td><?php echo !is_null($data["add_ons_id"]) ? $data["add_ons_name"] : "None" ;?></td>
                            <td>
                                ...
                                <button type="button" class="btn btn-success updateBtn" id="<?php echo $data["item_id"] ?>" data-order-type="<?php echo $data["order_type_id"] ?>" >
                  ...
                  <div class="card mb-5">
                    <div class="card-header bg-primary pt-3">
                        <div class="text-center">
                            <p class="card-title text-light">Pick up Orders To Claim
                        </div>
                    </div>
                    <div class="card-body">
                      <table id="toClaimOrders" class="table table-striped nowrap" style="width:100%">
                        ...
                        <tbody>
                          <?php
                            ...
                            $query = "SELECT tc.item_id, tc.account_id, tc.prod_id, tc.prod_qnty, 
                            tp.prod_name, tp.prod_price, 
                            CONCAT(ta.first_name, ' ', ta.middle_name, ' ', ta.last_name) as full_name, ta.contact, ta.address, ao.*, ot.*
                            FROM tbl_cart tc 
                            INNER JOIN tbl_products tp ON tp.prod_id = tc.prod_id  
                            INNER JOIN tbl_account_details ta ON ta.account_id = tc.account_id 
                            LEFT JOIN tbl_order_type ot ON ot.order_type_id = tc.order_type_id
                            LEFT JOIN tbl_add_ons ao ON ao.add_ons_id = tc.add_ons_id
                            WHERE tc.status_id = 4 AND ot.order_type_id = 1;";
                            $stmt = $conn->prepare($query);
                            $stmt->execute();
                            $result = $stmt->get_result();
                              while ($data = $result->fetch_assoc()) {
